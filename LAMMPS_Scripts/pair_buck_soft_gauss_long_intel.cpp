...
#include "pair_buck_soft_gauss_long_intel.h"
...
PairBuckSoftGaussLongIntel::PairBuckSoftGaussLongIntel(LAMMPS *lmp) :
  PairBuckSoftGaussLong(lmp)
{
  suffix_flag |= Suffix::INTEL;
}

void PairBuckSoftGaussLongIntel::compute(int eflag, int vflag)
{
...
void PairBuckSoftGaussLongIntel::eval(const int offload, const int vflag,
                                 IntelBuffers<flt_t,acc_t> *buffers,
                                 const ForceConst<flt_t> &fc,
      ...
      for (int ii = iifrom; ii < iito; ii += iip) {
        ...
        for (int jj = 0; jj < ej; jj++) {
          ...
          if (!ncoultablebits || rsq <= tabinnersq) {
            ...
            const flt_t alphaij = c_energyi[jtype].gwidth;
	    const flt_t alpha_rij  = alphaij*r;
            const flt_t t2 = 1.0/ (1.0 + EWALD_P*alpha_rij);
            const flt_t expm_gauss = exp(-alpha_rij*alpha_rij);
            const flt_t erfc_gauss = t2 * (A1+t2*(A2+t2*(A3+t2*(A4+t2*A5)))) * expm_gauss;
           
	    //fmt::print(screen, "{}, {}, {}, {}, {}\n", itype, jtype, alphaij, r, prefactor * (erfc - erfc_gauss));

	    if (EFLAG) {
		ecoul = prefactor * erfc; 
	    }
	    
            if (special_coul[sbindex] < 1.0) {
	    	const flt_t adjust = ((flt_t)1.0 - special_coul[sbindex])*prefactor;
	    	forcecoul -= adjust;
		if (EFLAG) ecoul -= adjust;
	    } else {
		forcecoul   -=  (erfc_gauss + EWALD_F*alpha_rij*expm_gauss)*prefactor;
		if (EFLAG) ecoul -= prefactor * erfc_gauss;
	    }
          ...
          #endif
            flt_t lambda = c_energyi[jtype].lambdas;
            flt_t r6inv = r2inv * r2inv * r2inv;
	    flt_t r6 = rsq * rsq * rsq;
            flt_t rexp = exp(-r * rho_invi[jtype]);
	    flt_t rlamb6sq = (c_energyi[jtype].c*(0.5-0.5*lambda) + r6) * (c_energyi[jtype].c*(0.5-0.5*lambda) + r6);
            forcebuck = lambda*c_forcei[jtype].buck1*r*rexp - lambda*c_forcei[jtype].buck2*r6/rlamb6sq;
            if (EFLAG) {
		    evdwl = lambda*c_energyi[jtype].a*rexp - lambda*c_energyi[jtype].c/(0.5*c_energyi[jtype].c*(1-lambda) + r6) - c_energyi[jtype].offset;
	    }
	    if (sbindex) {
	    ...
	    //fmt::print(screen, "{}, {}, {}, {}, {}\n", itype, jtype, r, forcebuck, forcecoul);
          #ifdef INTEL_VMASK
          }
          #else
          if (rsq > c_forcei[jtype].cut_ljsq)
...
void PairBuckSoftGaussLongIntel::init_style()
{
  PairBuckSoftGaussLong::init_style();
...
template <class flt_t, class acc_t>
void PairBuckSoftGaussLongIntel::pack_force_const(ForceConst<flt_t> &fc,
                                          IntelBuffers<flt_t,acc_t> *buffers)
{
  int off_ccache = 0;
  #ifdef _LMP_INTEL_OFFLOAD
  ...
  for (int i = 1; i < tp1; i++) {
    for (int j = 1; j < tp1; j++) {
      ...
      fc.c_energy[i][j].gwidth = g_width[i][j];
      fc.c_energy[i][j].lambdas = scaling_matrix[i][j];
...
void PairBuckSoftGaussLongIntel::ForceConst<flt_t>::set_ntypes(const int ntypes,
                                                           const int ntable,
                                                           Memory *memory,
                                                           const int cop) {
