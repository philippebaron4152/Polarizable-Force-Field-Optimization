...
def generate_data_lammps(p, o, m, b="", a="", einstein=False, triclinic=False):
    ri = []
    rf = []
    e, c = read_xyz(p)
        ...
        if triclinic:
            with open("box.xyz", 'r') as B:
                ...
                for k, line in enumerate(lines):
                    arr = np.array(line.split(), dtype=np.float64)
                    tri.append(arr[2])
                f.write(str(tri[0]) + ' ' + str(tri[1]) + ' ' + str(tri[2])  + '  xy xz yz\n')
        f.write('\n')
...
if __name__ == "__main__":
    ...
    argParser.add_argument('--triclinic', action=argparse.BooleanOptionalAction)
    args = argParser.parse_args()
    generate_data_lammps(args.xyz, args.out, a=args.angles, m=args.mixture, b=args.bonds, einstein=args.einstein, triclinic=args.triclinic)
